...
# Mapping from classify route to next node
_ROUTE_MAP = {
    "simple": "answer",
    "tool": "tool",
...
def route_after_approval(state: AgentState) -> str:
    ...
    # Handle both dict (from tests) and Pydantic model (from approval_node)
    if hasattr(approval, "approved"):
        approved = getattr(approval, "approved", False)
    else:
        approved = approval.get("approved") if isinstance(approval, dict) else False
